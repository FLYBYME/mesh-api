...
- [ ] **Cross-origin credentials — now more urgent, not less.** Each namespace has its own credential and auth does not leak across them (`12-network-and-federation.md`). But modern browsers' third-party cookie restrictions make cross-origin *cookie* sessions genuinely unreliable, so a token-based credential per namespace is the realistic answer, with cookies remaining correct same-origin. How a user's session on A becomes a credential for B — SSO, token exchange, or explicit second sign-in — must be settled before the first real federated deployment. Cookies alone will not carry it.
